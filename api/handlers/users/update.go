...
func UpdateMyProfile(cfg *config.AppConfig) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Only notify on changes to security-relevant contact info, not cosmetic
		// updates like avatar or referral code (audit M6).
		sensitiveChanged := req.Phone != "" || req.Address != ""
		if sensitiveChanged {
			go func() {
